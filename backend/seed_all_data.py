...
services_data = [
    {
        ...
        "fullDescription": "Our Foundational Agreements service helps couples establish clear expectations and understandings before marriage. Through structured sessions, we guide you in creating agreements around financial management, household responsibilities, career priorities, and life goals. This service ensures both partners enter marriage with aligned expectations and a shared vision for their future together."
    },
    {
        "id": "2",
...
async def seed_all_data():
    ...
    try:
        print("Seeding all website data to MongoDB...")
        
        # Clear existing data (except blogs which are already seeded)
        ...
        await db.team.insert_many(team_data)
        print(f"✅ Inserted {len(team_data)} team members")
        
        await db.faqs.insert_many(faq_data)
        print(f"✅ Inserted {len(faq_data)} FAQs")
