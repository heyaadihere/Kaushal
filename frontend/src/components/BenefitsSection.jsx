        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 max-w-7xl mx-auto">
          {benefits.map((benefit, index) => {
            ...
            return (
              <div
                key={index}
                className="card-3d bg-white/10 backdrop-blur-md rounded-2xl p-5 border border-white/20 hover:bg-white/20 transition-all duration-300 hover:-translate-y-2 group animate-scale-in stagger-${(index % 4) + 1} hover:shadow-2xl"
              >
                <div className="inline-flex p-2.5 rounded-xl bg-white/20 mb-4 group-hover:scale-110 transition-transform">
                  <Icon className="w-6 h-6 text-white" />
                </div>
                <h3 className="text-lg font-bold text-white mb-2">{benefit.title}</h3>
                <p className="text-xs text-white/90 leading-relaxed mb-4">{benefit.description}</p>
